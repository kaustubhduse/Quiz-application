      ...
      <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
        <div className="grid grid-cols-4 gap-3">
          {answers.map((a: any, i: number) => {
            ...
            let btnClass = "bg-white border-2 border-slate-200 text-slate-400 hover:border-blue-300 hover:text-blue-500"; 
            
            if(!isVisited){
                 btnClass = "bg-slate-50 border-2 border-slate-100 text-slate-400";
            } 
            else if(isAnswered){
                 btnClass = "bg-green-500 border-2 border-green-600 text-white shadow-md shadow-green-200"; 
            } 
            else if(isMarked){
                 btnClass = "bg-purple-500 border-2 border-purple-600 text-white shadow-md shadow-purple-200";
            } 
            else{
                 btnClass = "bg-red-50 border-2 border-red-200 text-red-500";
            }
            ...
            return (
              <button
                ...
                className={`
                    ...
                    ${btnClass}
                    ${isCurrent ? "ring-4 ring-blue-100 scale-110 z-10" : "hover:scale-105"}
                `}
              >
                {i + 1}
                {isMarked && isAnswered && (
